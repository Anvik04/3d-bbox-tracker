...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override camera height in meters",
    )
    parser.add_argument(
        "--tilt-deg", type=float, default=None, help="Override camera tilt in degrees"
    )
    parser.add_argument(
        "--fov-deg", type=float, default=None, help="Override camera FOV in degrees"
    )
    return parser
...
def main() -> None:
    ...
    if save_video:
        ...
        writer = cv2.VideoWriter(
            str(out_path), fourcc, 20.0, (frame_width, frame_height)
        )
    ...
    try:
        while True:
            ...
            for track in tracks:
                ...
                velocity = np.asarray(
                    track.get("velocity", [0.0, 0.0, 0.0]), dtype=float
                )
                ...
                centroid_history[track_id] = centroid_history.get(track_id, []) + [
                    centroid
                ]
            ...
            if overlay.shape[0] > 0 and overlay.shape[1] > 0:
                ...
                if bev is not None:
                    ...
                    overlay[:h, :w] = cv2.addWeighted(
                        overlay[:h, :w], 0.85, bev, 0.15, 0
                    )
